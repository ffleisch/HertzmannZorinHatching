...
public class GenerateCrosshatch : MonoBehaviour
{
    ...
    public Mesh generateMixedLineMesh()
    {
        ...
        foreach (var l in streamlines)
        {
            ...
            int lastStartIndex = vertices.Count;
            int i = 0;
            foreach(var sp in l)
            {
                bool isEndpiece =(i==0||i==(l.Count-1));

                vertices.Add(new Vector3(2*sp.pos.x/(w)-1, 2*sp.pos.y/(h)-1, sp.lineWidth*(isEndpiece?0:1)));//,
                endPiece.Add(new Vector2(isEndpiece?1:0,0));
            ...
            for (i = 0; i < n; i++)
            {
                Vector3 nextPoint = i < (n - 1) ? vertices[lastStartIndex+ i + 1] : vertices[lastStartIndex+ n - 1] + (vertices[lastStartIndex+ n - 1] - vertices[lastStartIndex+ n - 2]);
                Vector3 lastPoint = i > 0 ? vertices[lastStartIndex+ i - 1] : vertices[lastStartIndex] + (vertices[lastStartIndex] - vertices[lastStartIndex+ 1]);
